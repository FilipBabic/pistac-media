import Link from 'next/link';
import { useTranslations } from 'next-intl';
import Image from 'next/image';

import { Button } from '@/components/ui/button';

export default function Header() {
  const t = useTranslations('Home.Header');
  return (
    <header className="flex w-full flex-col items-center justify-center text-white md:flex-row">
      <section className="animate-slide-in-left flex flex-col items-center justify-center px-6 py-24 text-center">
        <h1 className="mb-4 text-4xl leading-tight font-extrabold text-[#b6b687] md:text-6xl">
          {t('h1')}
        </h1>
        <p className="mb-8 max-w-2xl text-lg text-gray-700 md:text-xl">
          {t('p-1')}
        </p>
        <Button
          asChild
          variant="outline"
          className="hover:bg-primary rounded-lg bg-white px-4 py-6 text-lg font-bold text-gray-700 transition duration-300 hover:text-white"
        >
          <Link href="/contact">{t('link')}</Link>
        </Button>

        <p className="mt-4 text-sm text-white">{t('p-2')}</p>
      </section>
      <div className="relative h-96 w-full rounded-2xl md:h-96 lg:h-[500px]">
        <Image
          src="/assets/images/mockup-tablet-1.jpg"
          alt="Women With Megaphone"
          fill
          sizes="(max-width: 768px) 100vw, (max-width: 1200px) 80vw, 50vw"
          className="animate-slide-in-left rounded-md object-contain"
          priority
        />
      </div>
    </header>
  );
}
